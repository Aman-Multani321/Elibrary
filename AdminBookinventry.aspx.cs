    ...
    public partial class AdminBookinventry : System.Web.UI.Page
    {
        ...
        void updateBookByID()
        {
            if (checkIfBookExists())
            {
                try
                {
                    ...
                    {
                        filepath = "~/Bookinventry/" + filename;
        ...
        void fillAuthorPublisherValues()
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("SELECT author_name from author_masther_tbl;", con);
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                DataTable dt = new DataTable();
                da.Fill(dt);
        ...
        void addNewBook()
        {
            try
            {
                ...
                string filepath = "~/Bookinventry/books1.png";
                string filename = Path.GetFileName(FileUpload1.PostedFile.FileName);
                FileUpload1.SaveAs(Server.MapPath("Bookinventry/" + filename));
                filepath = "~/Bookinventry/" + filename;
